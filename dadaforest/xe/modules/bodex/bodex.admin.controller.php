<?php
    ...
    class bodexAdminController extends bodex {
        ...
        /**
        * %brief 나중에 코어에서 문제가 수정될때 그땐 지우자 (phidel temp code)
        * 스킨 설정에 빠진 기본값 입력
        **/
        function _skinDefaultVars($skin_info, $out_args=null){
            ...
            if($out_args){
                ...
                foreach($out_args as $val) {
                    $skin_args->{$val->name} = $val->value;
                }
            }

            foreach($skin_info->extra_vars as $val) {
                //% 이미 설정된 값이 없고 기본값이 있으면
                if(!$skin_args->{$val->name} && $val->default){
                    //% 검색이 쉽게 배열 미리 정렬
         ...
         * @brief 사용자 확장 변수 등록
         **/
        function procBodexAdminAddExtraVar() {
        ...
        function procBodexAdminInsertBoard($args = null) {
            ...
            if(!in_array($args->use_reward,array('N','Y','R','D'))) $args->use_reward = 'N';
            ...
            if(!$args->use_reward_value) $args->use_reward_value = '10,30,50,100,300,500,1000';
