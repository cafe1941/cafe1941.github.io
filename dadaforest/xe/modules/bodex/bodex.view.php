    ...
    class bodexView extends bodex {
        ...
        function init() {
            ...
            if(!$this->module_info->use_reward){
                ...
                if(!in_array($this->module_info->use_reward,array('N','Y','R','D'))) $this->module_info->use_reward = 'N';
                ...
                if(!$this->module_info->use_reward_value) $this->module_info->use_reward_value = '10,30,50,100,300,500,1000';
            }
            ...
            //@ 스킨 경로를 미리 template_path 라는 변수로 설정함, 스킨이 존재하지 않는다면 xe_default로 변경
        ...
        function dispBoardContentList(){
            ...
            $args->sort_index = $sort_index = Context::get('sort_index');
            ...
            if($sort_index=='voted_count' && ($this->module_info->use_vote=='S'||$this->module_info->use_vote=='Z')) $sort_index='star_count';

            //% 상태의 검색 이거나 지원안하는 정렬이면 목록을 직접 구함
            if($args->sort_index == 'random' || $args->search_target=='doc_state' || ($sort_index && !in_array($sort_index, $this->order_target))){
                $args->sort_index = $sort_index?$sort_index:$args->sort_index;
                ...
                $output = $oDocumentModel->getDocumentList($args, $this->except_notice, $this->module_info->use_load_extra_vars == 'Y');
            }
        ...
        function dispBoardTagList() {
            //@ 만약 목록 보기 권한 없을 경우 태그 목록도 보여주지 않음
            ...
            $obj->list_count = ($list_count)?$list_count:10000;
        ...
        function dispBoardWrite() {
            ...
            //% 수정하기 포인트 사용시 답변이 달리면 관리자 외에 불가
