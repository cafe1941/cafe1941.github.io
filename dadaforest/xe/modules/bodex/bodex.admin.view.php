    ...
    class bodexAdminView extends bodex {
        ...
        function dispBodexAdminContent() {
            ...
            if($output->data && count($output->data)) {
                ...
                foreach($output->data as $key => $document) {
                    ...
                    $document->nick_name = ($output->data)?$output->data->nick_name:('MemberSrl_'.$document->member_srl);
        ...
        function dispBodexAdminBoardAdditionSetup() {
            ...
            if($logged_info->is_admin!='Y' && !$isSiteAdmin){
                ...
                * 추천/비추천 점수 초기화 방지를 위해 필드를 직접 입력해줌
                $oModuleModel = &getModel('module');
                $config = $oModuleModel->getModuleConfig('point');
                $point_config = $oModuleModel->getModulePartConfig('point', $this->module_info->module_srl);
                $content = preg_replace('/(<form\s.+insert_point_module_config.+>)/', '\\0<input type="hidden" value="'.$point_config['voted'].'" name="voted"><input type="hidden" value="'.$point_config['blamed'].'" name="blamed">', $content);
