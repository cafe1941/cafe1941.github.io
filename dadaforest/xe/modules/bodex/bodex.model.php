<?php
    ...
    class bodexModel extends module {
        ...
        function getListConfig($module_srl) {
            ...
            //% 구버전과 호환을 위해 사용 나중에 안정되면 지움 (phidel temp code)
            if(is_array($list_config)&&$list_config[0]){
                $list_arr = array();
                $sort_val = array( 'readed_count'=>true, 'regdate'=>true, 'last_update'=>true, 'voted_count'=>true);

                foreach($list_config as $key) {
                    if(preg_match('/^([0-9]+)$/',$key))
                        $list_arr['_'.$key] = 'N';
                    else
                        $list_arr[$key] = $sort_val[$key] ? 'Y':'N';
                }

                $list_config = $list_arr;
            }
        ...
        function getPointConfig($name = null, $module_srl = 0) {
            if(!$module_srl) $mid = Context::get('mid');
        ...
        function getNotAdoptedPostCount($module_srl=0) {
            $logged_info = Context::get('logged_info');
            if(!$logged_info->member_srl) return 0;
         ...
         * @brief 이전/다음 페이지 구하기
         **/
        function getDocumentNavigation($document_srl = 0, $module_srl = 0) {
            ...
            $excess_document = executeQuery('bodex.getDocument', $args);

            if($excess_document->data){
                $re['next'] = $excess_document->data;
            }

            unset($args->excess_document_srl);
            $args->order_type = 'asc';
            $args->below_document_srl = $document_srl;
            $below_document = executeQuery('bodex.getDocument', $args);

            if($below_document->data){
                $re['previous'] = $below_document->data;
            }
        ...
        function getDocumentList($obj, $except_notice = false, $load_extra_vars=true) {
            // 도큐먼트 모듈에서 지원되는 정렬대상 셋팅
            $arr_no_search_target = array('doc_state','best_voted_count','best_readed_count','best_comment_count');
            ...
            if(!in_array($obj->search_target, $arr_no_search_target) && in_array($obj->sort_index, $arr_order_target)){
            ...
            switch($sort_index) {
                ...
                case 'random' :
                    ...
                    break;
                default :
                        if(strpos($sort_index,'extra_vars')!==false) {
                            $args->sort_index = 'extra_vars.value';
                            $args->var_idx = substr($sort_index, strlen('extra_vars'));
                            $query_id = 'bodex.getDocumentListSortExtraVars';
                        }
                    break;
            }

            // 검색 옵션 정리
            if($search_target && ($search_keyword || $search_keyword === '0')) {
                switch($search_target) {
                    case 'doc_state' :
                            //if(!in_array($search_keyword, range('0','9'))) $search_keyword = '0';
                            if($search_keyword === '0') $search_keyword = "'0','N'";
