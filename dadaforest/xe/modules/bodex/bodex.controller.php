<?php
    ...
    class bodexController extends bodex {
        ...
        function _setUserInfo($sor_obj, $out_obj=null){
            //% sor_obj 에 유저 정보가 있다면
            if($sor_obj->member_srl){
        ...
        function _checkFileDownload($obj, $insert = false){
            ...
            if($module_info->use_reward != 'D') return new Object();
            ...
            $output = executeQuery('bodex.getFileDownloadedLogInfo', $args);
        ...
        function procBoardHistoryRestore(){
            ...
            //관리자와 글쓴이 본인이 아니면 오류
            $logged_info = Context::get('logged_info');
            if(!$logged_info->member_srl || (!$this->grant->manager && abs($history_data->member_srl) != $logged_info->member_srl)) return new Object(-1, 'msg_not_permitted');
            ...
            if(!$output->data->count){
                ...
                if($document_config->use_history == 'Y' || $document_config->use_history == 'Trace') {
                    $args->history_srl = getNextSequence();
                    $args->module_srl = $module_srl;
                    $args->document_srl = $oDocument->get('document_srl');
                    if($document_config->use_history == 'Y') $args->content = $oDocument->get('content');
                    $args->nick_name = $oDocument->get('nick_name');
                    $args->member_srl = $oDocument->get('member_srl');
                    $args->regdate = $oDocument->get('last_update');
                    $args->ipaddress = $oDocument->get('ipaddress');
            ...
            if($member_info && $history_data->member_srl > 0)
                $obj = $this->_setUserInfo($member_info);
            else{
        ...
        function procBoardInsertFileLink() {
            ...
            if(preg_match("/\.(png|jpg|jpeg|bmp|gif|ico|swf|flv|mp[1234]|as[fx]|wm[av]|mpg|mpeg|avi|wav|mid|midi|mov|moov|qt|rm|ram|ra|rmm|m4v)$/i", $filename)) {
                ...
                // 어차피 링크 파일이라 위험 없음...
                //if(preg_match("/\.(php|phtm|html|htm|cgi|pl|exe|jsp|asp|inc)/i",$filename)) return new Object(-1, Context::getLang('msg_invalid_upload_format')."\r\nnot: php, phtm, html, htm, cgi, pl, exe, jsp, asp, inc");
        ...
        function procBoardInsertDocument() {
            ...
            //% 익명 사용중인지 체크
            $is_anonymous = ($this->module_info->use_anonymous == 'R' || ($this->module_info->use_anonymous == 'Y' && $obj->is_anonymous == 'Y'));
            ...
            //% 포인트 글 작성전 먼저 에러 체크
            if($logged_info->member_srl && $this->module_info->use_reward != 'N'){
                //% 신규 글일때 미채택 글이 3개 이상이면 중단
                if($this->module_info->use_reward != 'D' && !$oDocument->isExists()){
                ...
                //% 포인트 필수일때 0과 같거나 작으면
                if($obj->reward_point <= 0 && ($this->module_info->use_reward == 'R' || $this->module_info->use_reward == 'D') ){
            ...
            if($oDocument->isExists()) {
                ...
                //% $old_reward_point 위에서 이미 호출
                //% $old_reward_point = $oDocument->get('reward_point');
                ...
                if($this->module_info->use_reward != 'N'){
                    ...
                    if($old_reward_point != 0 && ($logged_info->is_admin == 'Y' || $logged_info->member_srl == abs($oDocument->member_srl))){
                        $this->updateRewardPoint($output->get('document_srl'), $oDocument->member_srl, $obj->reward_point, $old_reward_point);
                    }
                }

                //% 게시물 익명 체크, 익명 정보 갱신
                if(Context::get('is_logged') && (abs($logged_info->member_srl) == abs($oDocument->member_srl)) && (($is_anonymous && $oDocument->member_srl > 0) || (!$is_anonymous && $oDocument->member_srl < 0))){
                    ...
                    if($document->toBool() && $document->data) {
                        $obj = $document->data;
                        if($is_anonymous)
                            $obj = $this->_setUserInfo($logged_info->member_srl, $obj);
                        else{
                            //% 위에서 로그인 유저와 해당 문서 유저가 같은지 체크하기에 맴버정보 구하는건 패스
                            //$oMemberModel = &getModel('member');
                            //$member_info = $oMemberModel->getMemberInfoByMemberSrl(abs($obj->member_srl), $this->module_info->site_srl);
                            //if(!$member_info) $member_info = $logged_info;
                            $obj = $this->_setUserInfo($logged_info, $obj);
                        }
                ...
                //% 익명 사용시 글의 글쓴이 정보를 모두 제거, 메일사용시 로그인 정보입력
                if($logged_info->member_srl && ($is_anonymous || $this->module_info->admin_mail)){
                    if($is_anonymous)
                        $obj = $this->_setUserInfo($logged_info->member_srl, $obj);
                    else
                        $obj = $this->_setUserInfo($logged_info, $obj);
        ...
        function procBoardInsertComment() {
            ...
            $is_anonymous = ($this->module_info->use_anonymous_comment == 'R' || ($this->module_info->use_anonymous_comment == 'Y' && $obj->is_anonymous == 'Y'));
            ...
            if($oComment->comment_srl != $obj->comment_srl) {
                ...
                if((abs($oDocument->get('member_srl'))!=abs($logged_info->member_srl)) && $this->module_info->use_vote != 'N'){
                    //% 0이 아니거나 필수일때는 들어감
                    if(($obj->vote_point && $obj->vote_point != 0) || $this->module_info->use_vote == 'Z' || $this->module_info->use_vote == 'R'){
                        $output = $this->updateDocumentVoteLog($obj->document_srl, $obj->vote_point);
                        if(!$output->toBool()) return $output;
                    }
                }
                ...
                //% 익명 사용시 글의 글쓴이 정보를 모두 제거, 메일 보내기시 로그인 정보입력
                if($logged_info->member_srl && ($is_anonymous|| $this->module_info->admin_mail_reply || $is_send_mail)){
                    if($is_anonymous)
                        $obj = $this->_setUserInfo($logged_info->member_srl, $obj);
                    else
                        $obj = $this->_setUserInfo($logged_info, $obj);
                ...
                //% 문제가 없고 모듈 설정에 관리자 메일이 등록되어 있거나 메일 알림이면 메일 발송 (익명사용시 위에서 이미 정보 제거됨)
            ...
            //@ comment_srl이 있으면 수정으로
                ...
                $oComment->member_srl = $oComment->get('member_srl');
                //@ 댓글 익명 체크, 익명 정보 갱신
                if(Context::get('is_logged') && (abs($logged_info->member_srl) == abs($oComment->member_srl)) && (($is_anonymous && $oComment->member_srl > 0)||(!$is_anonymous && $oComment->member_srl < 0))){
                    $oComment = $oCommentModel->getComment($output->get('comment_srl'), $this->grant->manager);
                    $obj = $oComment->getObjectVars();
                    if($is_anonymous)
                        $obj = $this->_setUserInfo($logged_info->member_srl, $obj);
                    else
                        $obj = $this->_setUserInfo($logged_info, $obj);
        ...
        function procBoardDeleteComment() {
            ...
            $this->updateAdoptCommentSrl($oDocument->get('document_srl'));
        ...
        function procBoardVerificationPassword() {
            ...
            //if(!$password) return new Object(-1, 'msg_input_password');
        ...
        **/
        function triggerDocumentMenu(&$obj){
            ...
            if($module_info->use_vote == 'Y'){
                ...
                $logged_info = Context::get('logged_info');
                if(abs($logged_info->member_srl) != abs($oDocument->get('member_srl'))) return new Object();
             }

            //% cmd_vote,cmd_vote_down
            $del_count = 0;
        ...
        **/
        function procBoardAdoptComment(){
            ...
            if(!$comment_srl) return new Object(-1, 'msg_invalid_request');
             ...
             $logged_info = Context::get('logged_info');
            if($logged_info->is_admin != 'Y' && (abs($doc_member_srl) != abs($logged_info->member_srl))) return new Object(-1, 'msg_not_permitted_act');
        ...
        **/
        function updateDocumentVoteLog($document_srl, $point){
            //% 문서번호가 없으면 에러
        ...
        **/
        function procBoardVoteRegister(){
            ...
            return $this->updateDocumentVoteLog($document_srl, $point);
        }
        ...
        **/
        function procBoardChangeState(){
            //% 관리자가 아니면 중단
            $oModuleModel = &getModel('module');
            $module_info = $oModuleModel->getModuleInfoByMid(Context::get('cur_mid'));
