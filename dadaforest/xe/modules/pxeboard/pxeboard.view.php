<?php
     ...
     * @brief  pxeboard 모듈의 View class
     **/

    class pxeboardView extends pxeboard {

        /**
         * @brief 초기화
         * pxeboard 모듈은 일반 사용과 관리자용으로 나누어진다.\n
         **/
        function init() {
            /**
        ...
        function dispBoardContentView(){
            ...
            if($document_srl) {
                ...
                if($oDocument->isExists()) {
                    ...
                    // get registrant rating
                    $oModel = &getModel('pxeboard');
                    $registrant_rating = $oModel->getRegistrantRating($document_srl);
                    $oDocument->add('registrant_rating', $registrant_rating->point);

                    // get readers rating
                    $reader_rating = $oModel->getReaderRatingAverage($document_srl);
                    $oDocument->add('reader_rating_average', $reader_rating->average);
                    $oDocument->add('reader_rating_count', $reader_rating->count);
                    ...
                    if ($output->data) {
                        $oDocument->add('file_srl', $output->data->file_srl);
                        // attach file
                        $oFileModel = &getModel('file');
                        $file = $oFileModel->getFile($output->data->file_srl);
                        if($file) $oDocument->add('download_url', getFullUrl().$file->download_url);

                        // screenshot
                        $oDocument->add('screenshot_url', $output->data->screenshot_url);
                        // screenshot link
                        $oDocument->add('screenshot_link', $output->data->screenshot_link);
                        // preview url
                        $oDocument->add('preview_url', $output->data->preview_url);
                        // label url
                        $oDocument->add('label_url', $output->data->label_url);
                    }
...
//            return new Object();
        ...
        function dispBoardContentList(){
            ...
            if (count($document_srl_list) > 0) {
                ...
                // getting registrant rating objects
                $registrant_rating = executeQueryArray('pxeboard.getRegistrantRating', $args);
                if ($registrant_rating->data) {
                    foreach($registrant_rating->data as $val) {
                        $documents[$document_index[$val->document_srl]]->registrant_rating = $val;
                    }
                }

                // getting readers rating objects
                $reader_rating = executeQueryArray('pxeboard.getReaderRatingAverage', $args);
                if ($reader_rating->data) {
                    foreach($reader_rating->data as $val) {
                        if ($val->point && $val->count) 
                            $val->average = round($val->point / $val->count);
                        else
                            $val->average = 0;

                        $documents[$document_index[$val->document_srl]]->reader_rating = $val;
                    }
                }

                // get attachment
                $attachments = executeQueryArray('pxeboard.getPxeboardAttachment', $args);
                if ($attachments->toBool() && $attachments->data) {
                    foreach ($attachments->data as $val) {
                        $documents[$document_index[$val->document_srl]]->file_srl = $val->file_srl;
                        // attach file
                        $oFileModel = &getModel('file');
                        $file = $oFileModel->getFile($val->file_srl);
                        if($file) $documents[$document_index[$val->document_srl]]->add('download_url', getFullUrl().$file->download_url);

                        // screenshot
                        $documents[$document_index[$val->document_srl]]->add('screenshot_url', $val->screenshot_url);
                        // screenshot link
                        $documents[$document_index[$val->document_srl]]->add('screenshot_link', $val->screenshot_link);
                        // preview url
                        $documents[$document_index[$val->document_srl]]->add('preview_url', $val->preview_url);
                        // label url
                        $documents[$document_index[$val->document_srl]]->add('label_url', $val->label_url);
                    }
                }
            }

             // get icons
            require_once("pxeboard.icons.php");
            $filter = array();
            foreach ($this->module_info as $key => $val) {
                if (substr($key, 0, 12) == 'icon_filter_' && $val == 'Y') 
                    $filter[] = substr($key, 12);
            }
            $icons = new pxeboardIcons($this->module_info->skin, $filter);
            Context::set('icons', $icons);
        ...
        function dispBoardWrite() {
            ...
            // if document exists, get rating and attachment infos.
            if ($oDocument->isExists()) {
                // registrant rating
                $oModel = &getModel('pxeboard');
                $rating = $oModel->getRegistrantRating($document_srl);
                $oDocument->add('registrant_rating', $rating->point);
                ...
                if ($output->toBool() && $output->data) {
                    ...
                    $file = $oFileModel->getFile($output->data->file_srl);
                    if($file) {
                        $oDocument->add('download_url', getFullUrl().$file->download_url);
                        $oDocument->add('download_file', $file);
                    }
        ...
        function dispPxeboardReaderRating() {
            $document_srl = Context::get('document_srl');
            $rating = Context::get('rating');

            // 세션 정보에 추천 정보가 있으면 중단
            if($_SESSION['pxeboard_rated'][$document_srl]) return new Object(-1, "msg_already_done");
            ...
            if($oDocument->get('ipaddress') == $_SERVER['REMOTE_ADDR']) {
                $_SESSION['pxeboard_rated'][$document_srl] = true;
                return new Object(-1, "msg_same_ipaddress");
            }
            ...
            if($oDocument->get('member_srl')) {
                ...
                if($member_srl && $member_srl == $oDocument->get('member_srl')) {
                    $_SESSION['pxeboard_rated'][$document_srl] = true;
                    return new Object(-1, "msg_same_person");
                }
            }
            ...
            $args->document_srl = $document_srl;
            $output = executeQuery('pxeboard.getReaderRatingAverage', $args);

            // 이미 평가했으면
            if($output->data->count) {
                $_SESSION['pxeboard_rated'][$document_srl] = true;
                return new Object(-1, "msg_alreay_rated");
            }

            // insert rating
            $args->document_srl = $document_srl;
            $args->rating = $rating;
            $oController = &getController('pxeboard');
            $oController->insertReaderRating($args);
            ...
            // get average
            $oModel = &getModel('pxeboard');
            $rating = $oModel->getReaderRatingAverage(Context::get('document_srl'));

            Context::set('reader_rating_average', $rating->average);
            Context::set('reader_rating_count', $rating->count);
        }

        // 파일 박스 보기
        function dispPxeboardLabelBox(){
            ...
            $oModel = &getModel('pxeboard');
            $output = $oModel->getLabelBoxList();
            Context::set('labelbox_list', $output->data);
            ...
            $this->setTemplateFile('labelbox_list');
        }

        // 파일 박스 등록화면
        function dispPxeboardLabelBoxAdd(){
            $logged_info = Context::get('logged_info');
            ...
            $this->setTemplatePath($this->module_path."tpl");
            $this->setTemplateFile('labelbox_add');
        }
