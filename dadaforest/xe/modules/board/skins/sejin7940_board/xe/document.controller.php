...
//  sejin7940 수정 (101023 - 시작) - 추천/비추천글 게시판 복사/이동 기능 ver.2
			$oModuleModel = &getModel('module');
...
//  sejin7940 수정 (101023 - 끝) - 추천/비추천글 게시판 복사/이동 기능 ver.2
...
//  sejin7940 수정 (101023 - 시작) - 신고글 게시판 복사/이동 기능 ver.2
			$oModuleModel = &getModel('module');
			$board_info = $oModuleModel -> getModuleSkinVars($oDocument->get('module_srl'));

			$declared_module_srl = $oModuleModel -> getModuleSrlByMid($board_info[declared_mid]->value);
			$declared_module_srl = $declared_module_srl[0];
			$declared_category = $board_info[declared_category]->value;
			$declared_document_srl[0] = $oDocument->get('document_srl');
			$declared_num = $board_info[declared_num]->value;
			if(($declared_count+1) == $declared_num ) {
				$oDocumentController = &getAdminController('document');
				if($board_info[voted_copymove]->value=='move') $oDocumentController -> moveDocumentModule ( $declared_document_srl, $declared_module_srl, $declared_category);
				else $oDocumentController -> copyDocumentModule ( $declared_document_srl, $declared_module_srl, $declared_category);
			}
//  sejin7940 수정 (101023 - 끝) - 신고글 게시판 복사/이동 기능 ver.2
