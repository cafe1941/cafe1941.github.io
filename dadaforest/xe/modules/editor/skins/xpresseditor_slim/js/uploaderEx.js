...
function XEUploaderStart(obj) {
    ...
    settings["fileCountAreaID"] = obj["fileCountAreaID"];
...
function uploadProgress(file, bytesLoaded, bytesTotal) {
    try {
        ...
        var fileCountObj = xGetElementById(this.settings["fileCountAreaID"]);
        ...
        if(fileCountObj) xInnerHtml(fileCountObj, percent+'%');
    } catch (ex)  {
        this.debug(ex);
    }
}
...
function completeReloadFileList(ret_obj, response_tags, settings) {
    ...
    var left_size = ret_obj["left_size"];
    ...
    var fileCountObj = xGetElementById(settings["fileCountAreaID"]);
    if(fileCountObj) xInnerHtml(fileCountObj, listObj.options.length.toString());

    var fileLeftSizeObj = xGetElementById('uploader_left_size_'+editor_sequence);
    if(fileLeftSizeObj) {
        if(left_size < 1024)
            attach_left_size = left_size + 'Byte';
        else if(left_size >= 1024 && left_size < 1048576)
            attach_left_size = (Math.round(parseFloat(left_size / 1024) * 100) / 100) + 'KB';
        else
            attach_left_size = (Math.round(parseFloat(left_size / 1048576) * 100) / 100) + 'MB';

        xInnerHtml(fileLeftSizeObj, attach_left_size.toString());
    }
...
function insertUploadedFile(editorSequence, styleOption) {
    var settings = uploaderSettings[editorSequence];
    var fileListAreaID = settings["fileListAreaID"];
    var fileListObj = xGetElementById(fileListAreaID);
    if(!fileListObj) return;
    ...
    for(var i=0;i<fileListObj.options.length;i++) {
        ...
        temp_code = temp_style = '';
        if(typeof(styleOption)!="undefined" && styleOption.length > 0 && styleOption!='center'){
            temp_style = "style=\""+styleOption+"\" ";
        }
        ...
        if(file.direct_download == 'Y') {
            ...
            if(/\.(jpg|jpeg|png|gif)$/i.test(file.download_url)) {
                ...
                temp_code += "<img "+temp_style+"src=\""+file.download_url+"\" alt=\""+file.source_filename+"\"";
                if(obj.complete == true) { temp_code += " width=\""+obj.width+"\" height=\""+obj.height+"\""; }
                temp_code += " />";
            // 이미지외의 경우는 multimedia_link 컴포넌트 연결
            } else {
                temp_code += "<img editor_component=\"multimedia_link\" multimedia_src=\""+file.download_url+"\" width=\"400\" height=\"300\" auto_start=\"false\" src=\""+request_uri+"/modules/editor/skins/xpresseditor_slim/img/multimedia_link.gif\" alt=\"\" />";
                if(temp_style.length > 0) temp_code = "<div "+temp_style+">"+temp_code+"</div>";
            }
        // binary파일의 경우 url_link 컴포넌트 연결
        } else {
            temp_code += "<a "+temp_style+"href=\""+file.download_url+"\">"+file.source_filename+"</a>";
        }

        if(temp_code) {
            if(typeof(styleOption)!="undefined" && styleOption=='center') temp_code = "<div style=\"text-align:center\">"+temp_code+"</div>";
            text.push(temp_code);
        }
    }
